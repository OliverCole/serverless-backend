...
var ssmparams = require('ssm-params');

// This is the standard entry method for lambdas.
exports.handler = (event, context, callback) => {

  // We prefixed the parameters with our Terraform env, which is also set
  // as an actual Lambda environment variable
  var ssmOpts = {
  ...
  // Load from SSM
  ssmparams.ssmToObj(ssmOpts, function(err, env_config) {
    var options = {
      auth: {
        // Mailgun API Key
    ...
    var email = {
      ...
      subject: 'Hello',
      text: 'This is the test email', // Email plaintext
      html: '<b>This is the test email in bold, and html!</b>' //Backup email HTML
    };
      ...
      if (err) {
        ...
        fs.readFile('./email.html', 'utf8', function(errmail, html) {
          if (errmail) {
            ...
            email.html = html.replace('RECIPIENT', raw_email);
            email.to = raw_email;
